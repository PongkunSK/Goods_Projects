...
<head>
    ...
    <style>
        ...
        /* FIXED: Card width and margin adjustments to prevent collision */
        .card {
            width: 300px;
            min-width: 300px;
            max-width: 300px;
        ...
        /* FIXED: Card slider layout to prevent collisions */
        .card-slider {
            display: flex;
            flex-wrap: wrap;
            justify-content: center;
        ...
        /* FIXED: Fixed column layout to prevent collisions */
        .col-md-3 {
            padding: 0;
            margin: 0 10px;
            display: flex;
        ...
        @media (max-width: 1400px) {
            ...
            /* FIXED: Card slider layout for smaller screens */
            .card-slider {
                flex-wrap: wrap;
                justify-content: center;
                gap: 20px;
            ...
            /* FIXED: Make cards take up proper width on mobile */
            .col-md-3 {
                display: flex;
                justify-content: center;
                flex: 0 0 auto;
...
<body>
                        ...
                        <div class="carousel-item <?= $first ? 'active' : '' ?>">
                            <div class="row justify-content-center card-slider">
                                <?php foreach ($post_group as $post): ?>
                                    <div class="col-md-3"> <!-- FIXED: Removed px-3 padding that was causing collision -->
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            // Initialize carousel with proper event handling
            ...
            // Fix for carousel control and indicator synchronization
            ...
            document.querySelectorAll('.carousel-control-prev, .carousel-control-next').forEach(control => {
                control.addEventListener('click', function() {
                    isControlClick = true;
                    setTimeout(() => { isControlClick = false; }, 700); // Reset after animation completes
                });
            });
            
            // Indicator button click handler with proper synchronization
            document.querySelectorAll('.carousel-indicators button').forEach(button => {
                button.addEventListener('click', function(e) {
                    ...
                    targetSlide = this.getAttribute('data-bs-slide-to');
                    
                    // Ensure we go directly to the target slide without animation accumulation
                    setTimeout(() => { 
            ...
            // Listen for slide events to update indicator states correctly
            carouselEl.addEventListener('slide.bs.carousel', function() {
                if (isIndicatorClick) {
                    // If indicator was clicked, we force the carousel to the intended slide
                    setTimeout(() => carousel.to(parseInt(targetSlide)), 0);
                }
            });
        });
        ...
        $(document).ready(function () {
            ...
            $(document).on("click", ".show-cards", function (e) {
                ...
                chunkedCards.forEach((group, index) => {
                    ...
                    let slide = `
                        <div class="carousel-item ${isActive}">
                            <div class="row justify-content-center card-slider">
                                ${group.map(card => `<div class="col-md-3">${card.outerHTML}</div>`).join("")}
                            </div>
                        </div>
                    `;
                ...
                // Reinitialize the carousel and fix event handlers
                const carouselEl = document.getElementById('customCarousel');
                const carousel = bootstrap.Carousel.getInstance(carouselEl) || new bootstrap.Carousel(carouselEl);
                carousel.to(0);
                
                // Reattach event handlers after rebuilding carousel
                let isControlClick = false;
                let isIndicatorClick = false;
                let targetSlide = 0;
                ...
                document.querySelectorAll('.carousel-control-prev, .carousel-control-next').forEach(control => {
                    control.addEventListener('click', function() {
                        isControlClick = true;
                        setTimeout(() => { isControlClick = false; }, 700);
                    });
                });
